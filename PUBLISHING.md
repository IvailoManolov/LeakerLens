...
- [ ] `version` in `package.json` bumped (currently **4.0.0**)
...
```sh
npm run package     # -> leaklens-4.0.0.vsix  (build + vsce package --no-dependencies)
```
...
```sh
code --install-extension leaklens-4.0.0.vsix
```
...
npx vsce publish --packagePath leaklens-4.0.0.vsix
...
```sh
npx ovsx publish leaklens-4.0.0.vsix -p <open-vsx-token>
```
...
```sh
git add -A
git commit -m "[Leak Lens] Release 4.0.0 — milestone + refreshed store presentation"
git tag v4.0.0
