...
- [ ] `version` in `package.json` bumped (currently **5.0.2**)
...
```sh
npm run package     # -> leakerlens-5.0.2.vsix  (build + vsce package --no-dependencies)
```
...
```sh
code --install-extension leakerlens-5.0.2.vsix
```
...
Requires a **publisher** (`leakerlens`) and an Azure DevOps **Personal Access Token** with the
*Marketplace → Manage* scope. See https://code.visualstudio.com/api/working-with-extensions/publishing-extension

```sh
# one-time: authenticate
npx vsce login leakerlens          # paste the PAT when prompted

# publish the already-built artifact
npx vsce publish --packagePath leakerlens-5.0.2.vsix
...
```sh
npx ovsx publish leakerlens-5.0.2.vsix -p <open-vsx-token>
```
...
```sh
git add -A
git commit -m "[LeakerLens] Release 5.0.2 — milestone + refreshed store presentation"
git tag v5.0.2
