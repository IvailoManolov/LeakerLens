...
```sh
npm run package     # -> leaklens-5.0.2.vsix  (build + vsce package --no-dependencies)
```
...
```sh
code --install-extension leaklens-5.0.2.vsix
```
...
npx vsce publish --packagePath leaklens-5.0.2.vsix
...
```sh
npx ovsx publish leaklens-5.0.2.vsix -p <open-vsx-token>
```
...
```sh
git add -A
git commit -m "[Leak Lens] Release 5.0.2 — milestone + refreshed store presentation"
git tag v5.0.2
